...
class BinaryReader {
    ...
    open func readNullTerminatedStringNoTrail() throws -> String {
      let string = try data.getNullTerminatedString(readIndex)
      readIndex = readIndex + string.utf8.count
      return string
    }
    
    open func readStringEncoded(_ data: BinaryReadableData) throws -> String {
    ...
    func read(_ length: Int) throws -> BinaryReadableData {
